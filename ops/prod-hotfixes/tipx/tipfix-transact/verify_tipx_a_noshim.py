...
def main():
    ...
    _proxy = _ClientProxy()
    # TIPFIX: shim DISABLED -- rail now uses ddb_transact_client() itself
    _ = _proxy  # noqa: F841
    ...
    try:
        ...
        # TIPFIX: the rail now uses its OWN bare client (ddb_transact_client), not
        # the shim _plain -- patch that real client so the forced-failure lands.
        from app.core.aws_clients import ddb_transact_client as _dtc
        _real = _dtc()
        saved = _real.transact_write_items
        def raise_tx(**kw):
            raise ClientError({"Error": {"Code": "ValidationException", "Message": "forced"}}, "TransactWriteItems")
        _real.transact_write_items = raise_tx
        try:
            try:
            ...
            _real.transact_write_items = saved
