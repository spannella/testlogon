...
async def _process_job_with_semaphore(job: Dict[str, Any]) -> None:
    ...
    Dispatches on ``job_type``: a ``download_mp4`` job (VOD-012 real progressive
    MP4 build) goes to ``process_download_mp4_job``; everything else (the ABR
    transcode default) goes to the existing ``execute_transcode_job`` — so
    standard transcodes are unaffected.
    """
    async with _CONCURRENCY_SEMAPHORE:  # type: ignore[union-attr]
        if job.get("job_type") == "download_mp4":
            await _execute_download_mp4_job(job)
        else:
            await execute_transcode_job(job)


async def _execute_download_mp4_job(job: Dict[str, Any]) -> None:
    """Claim + run a ``download_mp4`` job. Claiming is done here (not inside the
    processor) so the processor mirrors ``process_clip_job`` and the job store's
    running/complete/fail state machine stays consistent across workers."""
    job_id = job["job_id"]
    wid = _worker_id()
    if not claim_job(job_id, wid):
        logger.debug("download_mp4 job %s already claimed by another worker", job_id)
        return
    logger.info("Claimed download_mp4 job %s", job_id)
    from app.services.vod_mp4_generator import process_download_mp4_job

    await process_download_mp4_job(job)
