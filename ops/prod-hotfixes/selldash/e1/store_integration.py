...
def _bridge_fulfillment_status(order_id: str, user_sub: str):
    """ECOMX-E1: build OrderFulfillmentStatusOut from the REAL seller ship groups
    via order_fulfillment_bridge, scoped to the buyer's OWN order. Returns the
    populated model, or None when the order is not the buyer's / has no groups
    (so the caller falls back to the legacy path / empty shape). Never raises."""
    ...
    try:
        ...
        # Ownership: the buyer only ever sees their OWN order header. Use the
        # canonical composite-key header read (the legacy _get_order uses a
        # single-key GetItem that 400s on the composite orders table).
        header = _ol.get_order_header(order_id)
        if not header:
            return None
        owner = str(header.get("user_id") or header.get("user_sub")
                    or (header.get("metadata") or {}).get("user_id") or "")
        if owner != user_sub:
            return None
        data = _bridge.order_tracking(order_id, buyer_sub=user_sub)
        ...
        tracking = []
        for sh in shipments:
            tn = sh.get("tracking_number") or None
            groups.append(ShipGroupFulfillmentOut(
                ship_group_id=str(sh.get("ship_group_id") or ""),
                status=str(sh.get("status") or ""),
                items=[],
                carrier=(sh.get("carrier") or None),
                tracking_number=tn,
            ))
            if tn and tn not in tracking:
                tracking.append(tn)
        lifecycle_status = str(header.get("lifecycle_status")) if header.get("lifecycle_status") else None
        ...
        logger.debug("store_integration: bridge fulfilment join failed order=%s", order_id, exc_info=True)
        return None
...
def get_order_fulfillment_status(order_id: str, *, user_sub: str) -> Any:
    ...
    # ECOMX-E1: the buyer fulfilment view must populate off the REAL seller ship
    # groups (order_fulfillment_bridge) — the legacy SHP shipments table this
    # module used was never written by the seller-ship flow, so it always came
    # back empty. When order-lifecycle is on we ALWAYS attempt the bridge join
    # (scoped to the buyer), independent of the STORE_SHOW_FULFILLMENT_STATUS
    # sub-flag; the flag only still gates the legacy SHP path below.
    if getattr(S, "order_lifecycle_enabled", False):
        bridged = _bridge_fulfillment_status(order_id, user_sub)
