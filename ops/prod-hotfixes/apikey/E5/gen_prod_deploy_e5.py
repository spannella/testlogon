#!/usr/bin/env python3
"""Generate self-contained prod SSM probes for APIK E5 from the dev artifacts.
Run on the dev host (inside the venv):
  python gen_prod_deploy_e5.py place        > /tmp/prod_e5_place.py     # write artifacts + verify BEFORE
  python gen_prod_deploy_e5.py apply        > /tmp/prod_e5_apply.py     # backup 7 files + run idempotent patcher
  python gen_prod_deploy_e5.py verify_after > /tmp/prod_e5_after.py     # verify AFTER
Each emitted probe is run in-process on PROD via /tmp/ssm_run.py.
"""
import base64, os, sys

BASE = '/home/sean/dev/testlogon/ops/prod-hotfixes/apikey/E5'
FILES = ['apply_apik_e5_patch.py', 'verify_apik_e5.py', 'README.md']
enc = {}
for f in FILES:
...
TARGETS = [
    'app/routers/video_listing.py', 'app/routers/vod.py', 'app/routers/video_subtitles.py',
    'app/routers/transcode_jobs.py', 'app/routers/vod_bridge.py',
...
BASE = os.path.join(PROD, "ops/prod-hotfixes/apikey/E5")
FILES = %r
TARGETS = %r
...
sys.argv = ["verify"]
spec = importlib.util.spec_from_file_location("verifymod", os.path.join(BASE, "verify_apik_e5.py"))
...
spec = importlib.util.spec_from_file_location("applymod", os.path.join(BASE, "apply_apik_e5_patch.py"))
