#!/usr/bin/env python3
"""Generate self-contained prod SSM probes for APIK E3 from the dev artifacts.
Run on the dev host:
  python gen_prod_deploy_e3.py place        > /tmp/prod_e3_place.py     # write artifacts + verify BEFORE
  python gen_prod_deploy_e3.py apply        > /tmp/prod_e3_apply.py     # backup + apply patch on prod
  python gen_prod_deploy_e3.py verify_after > /tmp/prod_e3_after.py     # verify AFTER
Each emitted probe is run in-process on PROD via /tmp/ssm_run.py.
"""
import base64, os, sys

BASE = '/home/sean/dev/testlogon/ops/prod-hotfixes/apikey/E3'
FILES = ['apply_apik_e3_patch.py', 'verify_apik_e3.py', 'README.md']
enc = {}
for f in FILES:
...
BASE = os.path.join(PROD, "ops/prod-hotfixes/apikey/E3")
...
sys.argv = ["verify"]
spec = importlib.util.spec_from_file_location("verifymod", os.path.join(BASE, "verify_apik_e3.py"))
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
'''

APPLY = HEAD + '''
ts = int(time.time())
bak = REG + ".bak_apik_e3_%d" % ts
shutil.copy(REG, bak); print("BACKUP", bak)
spec = importlib.util.spec_from_file_location("applymod", os.path.join(BASE, "apply_apik_e3_patch.py"))
m = importlib.util.module_from_spec(spec)
os.environ["APIK_REG"] = REG
