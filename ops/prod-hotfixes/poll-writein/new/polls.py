"""Arbitrary text-option polls — surface-agnostic vote/results/close/write-in API.
...
def create_standalone_poll(body: PollCreateIn, session=Depends(require_ui_session)):
    return arbitrary_polls.create_poll(
        ...
        allow_write_in=body.allow_write_in,
        surface="standalone",
    )
...
@polls_router.post("/{poll_id}/write-in")
def write_in_poll(poll_id: str, body: PollWriteInIn, session=Depends(require_ui_session)):
    return arbitrary_polls.write_in(poll_id, session["user_sub"], body.text, body.question_id)
...
def poll_results(
    poll_id: str,
    question_id: Optional[str] = Query(default=None),
    top_n: Optional[int] = Query(default=5, ge=1, le=100),
    offset: int = Query(default=0, ge=0),
    session=Depends(require_ui_session),
):
    return arbitrary_polls.results(
        poll_id, session["user_sub"], question_id, top_n=top_n, offset=offset,
    )
