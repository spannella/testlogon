...
embeds a live snapshot at read time. Voting / results / close / write-in are
exposed surface-agnostically via ``app/routers/polls.py`` (``/ui/polls/...``).
...
def _snapshot(item: Dict[str, Any], viewer: Optional[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "allow_write_in": bool(pd.get("allow_write_in")) or bool(first_q.get("allow_write_in")),
...
def create_poll(
    *,
    ...
    q: Dict[str, Any] = {
        ...
        "allow_write_in": bool(allow_write_in),
    }
...
def write_in(poll_id: str, user: str, text: str, question_id: Optional[str] = None) -> Dict[str, Any]:
    item = _load(poll_id)
    if not item or not item.get("poll_data"):
        raise HTTPException(404, "Poll not found")
    qid = question_id or _first_qid(item)
    newsfeed_polls.add_write_in(item, poll_id, qid, text, user)
    return get_snapshot(poll_id, user)
...
def results(
    ...
    if not item or not item.get("poll_data"):
        raise HTTPException(404, "Poll not found")
    qid = question_id or _first_qid(item)
    return newsfeed_polls.get_poll_results(item, qid, viewer, top_n=top_n, offset=offset)
