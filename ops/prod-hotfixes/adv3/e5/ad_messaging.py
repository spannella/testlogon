...
# ── ADV3-10 (E7): global per-recipient sponsored-message frequency cap ──────
# A user must not be blanketed by MANY creators/advertisers, each of whom is
# individually under their own audience cap. A single per-recipient/day counter
# (across ALL sends + senders) caps how many sponsored messages one inbox
# receives per UTC day. Read-then-act (a mild TOCTOU is acceptable for a soft
# abuse limit, mirroring the per-user account limit) + fail OPEN so a counter
# error never blocks legitimate delivery.
def _freq_pk(recipient_sub: str) -> str:
    return "AMSGFREQ#%s" % recipient_sub
...
def recipient_daily_cap() -> int:
    from app.core.settings import S
    return int(getattr(S, "ad_msg_recipient_daily_cap", 5) or 0)


def recipient_over_freq_cap(recipient_sub: str) -> bool:
    """True when this inbox has already hit the global daily sponsored-message
    cap (across all senders). Fails OPEN (allow) on any read error."""
    cap = recipient_daily_cap()
    if cap <= 0 or not recipient_sub:
        return False
    try:
        item = T.sponsorship_deals.get_item(
            Key={"pk": _freq_pk(recipient_sub), "sk": "DAY#%s" % _freq_day()}
        ).get("Item") or {}
        return int(item.get("count", 0) or 0) >= cap
    except Exception:
        return False


def _bump_recipient_freq(recipient_sub: str) -> None:
    """Atomically count one delivered sponsored message for the recipient/day."""
    ...
    try:
        T.sponsorship_deals.update_item(
            Key={"pk": _freq_pk(recipient_sub), "sk": "DAY#%s" % _freq_day()},
            UpdateExpression=(
                "SET #c = if_not_exists(#c, :z) + :one, updated_at = :ts, "
                "expires_at = if_not_exists(expires_at, :ttl)"
            ),
            ExpressionAttributeNames={"#c": "count"},
            ExpressionAttributeValues={":z": 0, ":one": 1, ":ts": ts, ":ttl": ts + 172800},
        )
    except Exception:
        logger.warning("ad_msg_recipient_freq_bump_failed recipient=%s", recipient_sub)
...
def approve_and_send(
    *, offer_id: str, creator_sub: str, body_override: Optional[str] = None,
    ...
    # ADV3-10 (E7): a sponsored DM previously hit up to 500 inboxes with NO
    # platform content review + NO enforced label. Screen the (possibly
    # creator-overridden) body BEFORE claiming the offer so a HARD violation 422s
    # without stranding the offer in "approved"; force a non-empty disclosure
    # label server-side (below, threaded into every delivered message).
    ...
    if getattr(_S, "ad_policy_screening_enabled", True):
        try:
            from app.services.ad_policy import screen_creative
            _pr = screen_creative(body_text=body, cta_text=offer.get("cta_url") or "",
                                  cta_url=offer.get("cta_url") or "")
            _hard = _pr.hard_violation
            _pmeta = _pr.to_dict()
        except Exception:
            _hard, _pmeta = False, {}
        if _hard:
            raise AdMessagingError(
                422, {"code": "policy_violation",
                      "message": "Message content violates ad policy and cannot be sent.",
                      "policy": _pmeta})

    # Server-forced disclosure label (never advertiser-optional).
    _default_label = getattr(_S, "ad_default_disclosure_label", "Paid partnership")
    offer = dict(offer)
    offer["sponsor_label"] = str(offer.get("sponsor_label") or "").strip() or _default_label
    ...
    *, product: str, offer: Dict[str, Any], creator_sub: str, sender_sub: str,
    ...
    for recipient in recipients:
        if eligibility_fn is not None and not eligibility_fn(recipient):
            ...
            continue
        # ADV3-10 (E7): global per-recipient daily frequency cap -- an inbox that
        # has already hit the cap across all senders is DROPPED (no charge, no
        # send), independent of this creator's own audience cap.
        if recipient_over_freq_cap(recipient):
            results.append({"recipient_sub": recipient,
                            "state": "excluded_freq_cap", "charge_cents": 0})
            continue
        ...
        results.append(r)
        if r.get("state") == "sent":
            # Count a real delivery toward the recipient's global daily cap.
            _bump_recipient_freq(recipient)
