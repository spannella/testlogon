...
# ADV3-12 (E8): allowed admin review decisions. An unknown/typo'd decision must
# NOT be written verbatim as the account status ("banana" -> status="banana"
# left the account in an undefined state that neither served nor counted as
# suspended). Validated here so EVERY caller (router, admin_ad_platform.moderate,
# api-key path) is protected, not just the Pydantic HTTP model.
ALLOWED_ACCOUNT_DECISIONS = frozenset({"approve", "reject", "suspend"})


def review_ad_account(
    account_id: str, reviewer_sub: str, decision: str, notes: str = ""
) -> Optional[dict]:
    if decision not in ALLOWED_ACCOUNT_DECISIONS:
        raise ValueError(
            "invalid account review decision: %r (allowed: %s)"
            % (decision, ", ".join(sorted(ALLOWED_ACCOUNT_DECISIONS)))
        )
