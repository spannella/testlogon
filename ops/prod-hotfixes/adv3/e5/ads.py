...
async def review_account_endpoint(
    ...
    except ValueError as exc:  # ADV3-12 (E8): unknown decision -> 422
        raise HTTPException(status_code=422, detail=str(exc))
    if not result:
        raise HTTPException(status_code=404, detail="Account not found")
    return result
...
async def list_pending_creatives(user: AuthenticatedUser = Depends(require_admin_or_root)):
    # ADV3-11 (E1): risk-rank the queue so the admin sees the highest ad-policy
    # score first (a clean creative has policy_score 0 / absent -> sorts last).
    rows = list_creatives_by_status("pending_review")
    rows.sort(key=lambda c: int(c.get("policy_score", 0) or 0), reverse=True)
    return rows
...
async def analytics_summary_endpoint(
    ...
    # ADV3-8 (D1): honor the app's from/to window (was silently ignored -> every
    # KPI stuck at 30d). Falls back to days when from/to are absent (web path).
    from app.services.ad_analytics import get_summary
    _require_account_owner(account_id, ctx["user_sub"])
    return get_summary(account_id, campaign_id, days, from_date=from_, to_date=to)
...
async def analytics_timeseries_endpoint(
    ...
    # ADV3-9 (D8): only hourly/daily rows are materialized. Reject weekly/monthly
    # with 422 instead of silently returning an empty chart.
    if granularity in ("weekly", "monthly"):
        raise HTTPException(422, "Granularity weekly/monthly is not supported yet; use daily")
    return get_timeseries(account_id, campaign_id, days, granularity, from_date=from_, to_date=to)
...
async def analytics_breakdown_endpoint(
    ...
    return get_breakdown(account_id, campaign_id, dimension, days, from_date=from_, to_date=to)
