...
def _campaign_targeting_countries(campaign_id: str) -> list:
    """ADV3-12 (E9): union of the country codes a campaign targets, so the fraud
    engine's geo-mismatch rule can actually fire on the live track/CTA path
    (previously check_fraud was called with NO targeting_countries, so
    _check_geo_mismatch always scored 0). Best-effort; empty list on any error
    just leaves the geo rule inert (its prior behaviour)."""
    if not campaign_id:
        return []
    try:
        from app.services.ad_targeting import list_targeting_sets
        out: set = set()
        for ts in list_targeting_sets(campaign_id) or []:
            for c in (ts.get("country_codes") or []):
                if c:
                    out.add(str(c).upper())
        return sorted(out)
    except Exception:
...
def serve_ad(
    *,
    ...
    for campaign in active_campaigns:
        ...
        # ADV3-12 (E6): the serve gate must require the ad account be ACTIVE, not
        # merely "not suspended". A rejected / pending / undefined-status account
        # (e.g. after moderate reject, which previously did NOT cascade-pause its
        # campaigns) kept serving + billing until each campaign was individually
        # paused. Require status == "active" here so a reject stops fill
        # immediately. Subsumes the old suspended-only check. Fail SAFE: an
        # unresolvable account does not serve.
        if str((_acct or {}).get("status", "")) != "active":
            continue
        ...
        if getattr(S, "ad_fraud_detection_enabled", True):
            try:
                ...
                fr = fraud.check_fraud(
                    ...
                    targeting_countries=_campaign_targeting_countries(campaign_id),
                ...
                # ADV3-12 (E9): record activity even on a scoring error.
                try:
                    from app.services import ad_fraud_prevention as _fr
                    _fr.record_account_activity(account_id=account_id, flagged=False)
                except Exception:
                    pass
        try:
    ...
    if getattr(S, "ad_fraud_detection_enabled", True):
        try:
            ...
            result = fraud.check_fraud(
                ...
                # ADV3-12 (E9): thread the campaign's targeting countries so the
                # geo-mismatch rule actually fires on the live path.
                targeting_countries=_campaign_targeting_countries(campaign_id),
            )
            ...
            # ADV3-12 (E9): even when scoring ERRORS, record the account activity
            # so the fraud-rate denominator is not silently understated (the
            # except previously skipped both the flag AND record_account_activity).
            try:
                from app.services import ad_fraud_prevention as _fr
                _fr.record_account_activity(account_id=account_id, flagged=False)
            except Exception:
