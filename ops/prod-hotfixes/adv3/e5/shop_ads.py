...
def boost_listing(
    *,
    ...
    # 3. product creative prefilled from the listing. ADV3-10 (E2): the
    #    boost creative previously shipped status="approved" and served
    #    IMMEDIATELY with ZERO review -- the concrete "anything serves
    #    unreviewed" hole. It is now routed through the SAME automated ad-policy
    #    pass as a submitted creative (keep the account auto-provision): a HARD
    #    violation is auto-rejected (never serves); a risk result is held in
    #    pending_review for a human; only a CLEAN self-serve boost auto-approves.
    ...
    if getattr(_S, "ad_policy_screening_enabled", True):
        try:
            ...
            pr = screen_creative(
                title=name, headline=headline, body_text=body_text,
                cta_text="Shop now", category="general",
            )
            policy_meta = pr.to_dict()
            if pr.decision == "reject":
                creative_status = "rejected"
            elif pr.decision == "review":
                creative_status = "pending_review"
            else:
                creative_status = "approved"
        except Exception:
            logger.warning("boost_policy_screen_failed item=%s", item_id)
            creative_status = "pending_review"  # fail SAFE: hold, do not auto-serve
    ...
    if policy_meta:
        try:
            T.ad_creatives.update_item(
                Key={"pk": f"CAMP#{campaign_id}", "sk": f"CREATIVE#{creative['creative_id']}"},
                UpdateExpression=(
                    "SET policy_decision = :pd, policy_score = :ps, "
                    "policy_flags = :pf, policy_reasons = :pr"
                ),
                ExpressionAttributeValues={
                    ":pd": policy_meta.get("policy_decision", ""),
                    ":ps": int(policy_meta.get("policy_score", 0) or 0),
                    ":pf": policy_meta.get("policy_flags", []),
                    ":pr": policy_meta.get("policy_reasons", []),
                },
            )
        except Exception:
            logger.warning("boost_policy_meta_stamp_failed creative=%s", creative["creative_id"])
    ...
    return {
        ...
        "status": "active",
        # ADV3-10: the boost no longer implies "serving now" -- the creative must
        # clear the policy/review gate first. Surface both so the caller (app)
        # can tell the seller "in review" vs "live".
        "creative_status": creative_status,
        "serving": creative_status == "approved",
        **({"policy": policy_meta} if policy_meta else {}),
    }
