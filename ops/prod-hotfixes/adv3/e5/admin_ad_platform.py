...
def moderate_account(
    *, account_id: str, action: str, admin_sub: str,
    ...
    # ADV3-12 (E6): both suspend AND reject must cascade-pause. Previously only
    # suspend cascaded, so a rejected account's campaigns kept their "active"
    # status; with the serve gate now requiring status=="active" the account
    # stops serving immediately, but pausing the campaigns keeps the campaign
    # state machine consistent (no orphaned "active" campaign under a non-active
    # account) and stops any per-campaign path that keys off campaign status.
    if action in ("suspend", "reject"):
