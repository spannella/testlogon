...
@router.post("/ui/admin/shipment-tracking/run-progression")
async def run_progression(
    limit: Optional[int] = Query(default=None, ge=1, le=500),
    ctx: Dict[str, str] = Depends(require_ui_session),
) -> Dict[str, Any]:
    """ECOMX-30 admin run-now: advance every in-flight shipment ONE honest step
    (EasyPost poll when keyed; internal mock progression otherwise). Deterministic
    trigger for the same driver the background runner uses."""
    if ctx.get("role") not in ("admin", "root"):
        raise HTTPException(403, {"code": "admin_required"})
    from app.services.shipment_tracking_runner import run_now
    return run_now(limit)
