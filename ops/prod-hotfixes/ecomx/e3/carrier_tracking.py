...
# ── Carrier Detection (ECOMX-31: single source of truth) ──────────
#
# Detection is DELEGATED to shipment_tracking.detect_carrier so there is ONE
# carrier detector in the codebase. That impl returns UPPERCASE carrier codes
# (USPS/UPS/FedEx/DHL / unknown); this module speaks lowercase (ups/fedex/
# usps/dhl) to match CARRIER_TRACKING_URLS + the buyer-txn shipping records, so
# we map the result back down.

_ST_TO_LOWER: Dict[str, str] = {
...
def detect_carrier(tracking_number: str) -> Optional[str]:
    ...
    ECOMX-31: delegates to the canonical shipment_tracking.detect_carrier and
    maps its UPPERCASE result to this module's lowercase vocab. Returns None when
    the format is unrecognised (unchanged contract for callers).
    """
    if not tracking_number or not str(tracking_number).strip():
        return None
    from app.services.shipment_tracking import detect_carrier as _st_detect
    return _ST_TO_LOWER.get(_st_detect(tracking_number))
