...
| `NEWSFEED_RECSYS_ENABLED` | Newsfeed "For You" ranked feed (default **off**; off → `GET /feed/for-you` returns the chronological feed unchanged) |
...
**KYC Partner API rate limiting + document S3 storage (GAP-0286 + GAP-0287, KYC-021)**: (GAP-0286) Every `app/routers/kyc_partner_api.py` handler now calls `rate_limit_kyc_partner_api(api_key_id, category)` (new helper in `app/services/rate_limit.py`, keyed on `apikey:{api_key_id}` so each issued key gets its own budget — NOT keyed on the partner `user_sub`). Categories + hourly caps per KYC-021 §4.7: `applications`=100 (POST /applications, /submit), `documents`=200 (POST .../documents), `read`=1000 (all GET incl. GET /webhooks + the new download endpoint), `webhook_test`=10 (POST /webhooks/{id}/test). Caps are env-configurable via `KYC_PARTNER_API_RL_*` settings; uses the existing DDB-backed `_bucket_limit` (window=3600), so 429 carries `code=kyc_api_rate_limited` + `Retry-After`. (GAP-0287) `svc.upload_document` previously read the multipart bytes and discarded them; it now calls `_store_document_bytes(...)` which `put_object`s to S3 at `{S.kyc_partner_api_docs_s3_prefix}{partner_id}/{application_id}/{document_id}/{safe_filename}` via the centralized `app.core.aws_clients.s3_client` factory, records `s3_key`/`bucket` on the document dict, and (dev) returns a `/mock/s3/...` `url`. New `GET /api/v1/kyc/applications/{id}/documents/{doc_id}` → `svc.get_document_download` returns the dev mock URL or a 300s presigned URL (prod), enforcing per-partner isolation (only the owning partner's `PARTNER#{id}` partition is read → 404 otherwise). Same code path dev (in-process moto) + prod (real S3/DDB) — no `dev_mode` bypass (SECOPS-007 parity). Regression test: `tests/test_gap_0286_0287_kyc_partner_api.py` (offline/hermetic; moto in-memory `kyc_cases`/`sessions` tables bound to the exact frozen `T.kyc_cases`/`T.sessions` handles via `object.__setattr__`, restored on cleanup; S3 client patched to an in-memory `_FakeS3` so no real AWS / no moto-S3 global interception; frozen `S` caps/flags toggled via `object.__setattr__`; async route handlers invoked directly on a fresh `asyncio.new_event_loop()`).

**Messenger Voice & Translation AI (MVA-001..MVA-009)**: ElevenLabs is now a registered provider in `PROVIDER_REGISTRY` (`app/services/llm_provider_keys.py`) with `auth_header="xi-api-key"`, `test_endpoint="/voices"`, TTS `models`, `stt_model="scribe_v1"`, `default_voice_id`; `add_key` accepts `voice_preference` (defaults to the registry `default_voice_id`) and `test_key` parses the ElevenLabs `/voices` shape in prod (dev returns the mock model list). Provider callers live in `app/services/messaging_ai.py` — `translate_text` (Anthropic Messages API, env `ANTHROPIC_API_KEY` fallback), `transcribe_audio` (ElevenLabs `/speech-to-text`), `synthesize_speech` (ElevenLabs `/text-to-speech/{voice}`); each is `S.dev_mode`-aware (dev returns deterministic mocks, NO SDK/network) and records cost via `record_usage`. Three flag-gated endpoints in `app/routers/messaging.py` (appended at file end): `POST /conversations/{cid}/messages/{mid}/translate` (gated `messaging_translation_enabled`, default ON; cached in `T.message_ai_cache` keyed `sha256(mid|target_lang|text_hash)`; cache-hit short-circuits + records no cost), `POST .../{mid}/transcribe` (gated `messaging_transcription_enabled`, default OFF; persists `transcript`/`transcript_lang` on the message item, idempotent), `POST /conversations/{cid}/tts-voice-message` (gated `messaging_tts_enabled` + `voice_message_enabled`, default OFF; server uploads MP3 to `voice-messages/{cid}/{mid}.mp3` and creates a `kind=voice_message` item with `is_tts=true`). Per-feature DDB rate limits live on `T.message_ai_cache` (`_ai_rate_limit_or_429`, fail-open) → 429 + `Retry-After`. New `message_ai_cache` table (PK `cache_key`, numeric `created_at`/`ttl`) in `scripts/local-ddb-init.py` + `T.message_ai_cache`. `_message_out_from_item` voice-message projection now surfaces `transcript`/`transcript_lang` + TTS provenance; `MessageOut` gains an additive `translation` field (MVA-005 projection wiring is a follow-up). Regression test: `tests/test_messenger_voice_ai.py` (offline/hermetic; dict-backed `_FakeTable` bound to frozen `T.message_ai_cache`/`T.llm_provider_keys` + the router's `tbl_msgs`/`tbl_parts`/`tbl_convos`/`s3`, KMS + heavy messaging collaborators patched, dev-mode mock provider branch — no AWS/network/SDK). Follow-ups: auto-translate preference + projection (MVA-005), all frontend (MVA-006/008/010).

**Newsfeed "For You" ranked feed (NRS-001..NRS-010, flag default OFF)**: `app/services/newsfeed_recsys.py` adds a ranked feed reusing the video recsys patterns in `app/services/recommendations.py` and the SAME `recommendations` DDB table (pk/sk single-table). Gated entirely behind `S.newsfeed_recsys_enabled` (`NEWSFEED_RECSYS_ENABLED`, default false). With the flag off everything is a no-op: `GET /feed/for-you` (`app/routers/newsfeed.py:view_for_you_feed`) delegates to the existing `view_feed` and returns the chronological body + `source="chronological_fallback"`; the engagement hooks are inert. Data rows (namespaced so video `VIDEO#`/`FOR_YOU` rows never collide): `SIGNAL#{user}`/`POST#{post_id}` (per-user post engagement, carries `author_id`+weighted `signal_score`+TTL), `RECO#{user}`/`FOR_YOU_POSTS` (pre-computed ordered ids, 24h TTL), `POPULAR#GLOBAL`/`POST#{post_id}` (sparse out-of-network popularity index, TTL). Signal recording is wired fire-and-forget via `_recsys_engage(...)` into like/reaction/comment/tip/unlock in `newsfeed.py` (all best-effort, gated by the flag → zero effect when off). `score_post` is pure/deterministic (recency decay + engagement velocity + author affinity + content-type + personal-history weights, all env-tunable `NEWSFEED_RECSYS_WEIGHT_*`); `rank_candidates` applies a caller-supplied `exclude` callback so the router keeps owning block/snooze/hide/moderation/unpublished/locked-visibility parity. `generate_candidate_ids` merges 3 sources (followed via GSI1 feed refs, popular via the new index = the out-of-network/fan-out gap fix, affinity via signals+follows → GSI2 author posts) with injected `fetch_followed`/`fetch_author_posts` collaborators for hermetic testing. `compute_for_you_posts` returns `source="cold_start"` for a viewer with no signals AND no follows, else `for_you`. Background refresh: `start_newsfeed_recsys_refresh_task` (registered in `main.py`, gated on flag) + internal `POST /internal/newsfeed-recsys/refresh` (single viewer or all signal users). `GET:/feed/for-you` added to `API_KEY_ROUTE_EXEMPTIONS`. Test: `tests/test_newsfeed_recsys.py` (offline; moto `recommendations` table bound to frozen `T.recommendations` via `object.__setattr__`, `now_ts`/`time.time` patched, route handler called directly). NOTE: `tests/test_newsfeed_routes.py` requires a live DDB/moto backend not present in a bare pytest run (fails with AccessDenied against real AWS — pre-existing, unrelated to recsys).
