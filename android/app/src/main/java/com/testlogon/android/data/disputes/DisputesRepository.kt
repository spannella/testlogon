...
/**
 * AND-245 — disputes data layer over [DisputesApi].
 *
 * Wraps every call in [ApiResult]: CancellationException is re-thrown; HTTP errors fold to Failure (via
 * [ApiErrorParser]); transport failures to NetworkError. DTOs are mapped to domain before returning. The
 * list is sorted newest-first by created_at (the backend gives no ordering guarantee — AND-245 FR-3).
 *
 * GET reads (list/detail) are idempotent; the file-dispute POST is non-idempotent (no auto-retry).
 */
interface DisputesRepository {

    /** All of my disputes (single bounded fetch, newest-first). Idempotent GET. */
