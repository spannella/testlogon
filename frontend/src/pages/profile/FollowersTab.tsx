...
import { getFollowers } from "@/api/endpoints/social";
...
export function FollowersTab({ userId }: FollowersTabProps) {
  ...
  const {
    ...
    queryFn: ({ pageParam }: { pageParam: string | undefined }) =>
      getFollowers(userId, pageParam ? { cursor: pageParam, limit: "20" } : { limit: "20" }),
  ...
  return (
    <div className="space-y-2" role="list" data-testid="followers-list">
      {allItems.map((user) => (
        <div
          key={user.user_id}
          role="listitem"
