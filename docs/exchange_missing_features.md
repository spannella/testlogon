...
### Tokenized creator revenue-share contracts — ✅✅ SHIPPED + DEPLOYED TO PROD (2026-08-21) — see "Added 2026-08-21 (pm)"
> **STATUS FLIP:** this entire epic (backend + engine) is now built, tested, and live on prod. The spec below is retained as the as-built reference; all §B–I are implemented. Remaining work is the 4 hardening follow-ups + secondary-market gaps listed in the 2026-08-21 (pm) section.
...
### External custody providers (Fireblocks / BitGo / …) — 📋 BACKLOG (requested 2026-08-21, not started)
> *Read "firebase" in the request as **Fireblocks** — the institutional custody provider paired with BitGo.*

Back the custody layer with an **external qualified custodian** (Fireblocks, BitGo, Anchorage, …) instead of only the internal gateway, so balances/keys can be held by a regulated third party with policy-governed withdrawals.
- **Reuses:** the existing custody surface (vaults, deposit address+QR, withdraw, sub-accounts, deposits history) — add a **provider abstraction** behind it so a vault is backed by `internal | fireblocks | bitgo`.
- **Hard backend blockers:** a **provider adapter** per custodian (Fireblocks / BitGo REST + webhook APIs); external **address derivation** per asset/vault; **deposit webhook ingestion** → credit custody; **withdrawal signing** routed through the provider's policy/MPC engine (approval quorums, allow-lists); balance **reconciliation** (provider-attested vs internal ledger); and secure storage of provider **API creds** (never client-side — reuse the server-to-server HMAC pattern).
- **Frontend (buildable now, degrade-on-404):** a **Connect custody provider** flow (link a provider account / API creds server-side), a per-vault **provider selector + attestation badge** (who custodies this vault), provider-routed deposit/withdraw with the provider's approval-status surfaced, and a provider health/status indicator. Mirrors the credential-management + custody UIs already shipped.
- **Open decisions (lock before build):** which provider(s) first; org-level vs per-user custody accounts; MPC vs multisig; delegated vs self-custody; whether the internal gateway stays as a fallback.

### User-created strategies / baskets (ETF-like + rule-based, investable) — 📋 BACKLOG (requested 2026-08-21, not started)
A marketplace of **user-authored strategies**: a creator defines a **basket following a simple rule set**; others can **paper-trade** it, view its **backtest**, then **invest** real capital. Spans a static **ETF-like basket** (fixed target weights + periodic rebalance) through a **complex conditional rule strategy**.
- **Creator-set parameters (all required):** **minimum investment size**; **maximum total AUM** (capacity cap); **redemption / profit-taking policy** (instant-at-NAV vs notice period / lockup / gate; how profit is taken); and the **fee schedule** — a **management fee on AUM** (e.g. 2%/yr, accrued daily) **and** a **performance fee on profit** (e.g. 20% carry, with a **high-water mark**).
- **Lifecycle:** draft → **paper-trade + backtest** (reuse the paper engine + the `/analysis` backtest math already shipped) → publish → accept investments (enforce min-size + capacity) → **run** (rebalance per the rules) → investors **subscribe/redeem at NAV** → fees **accrue** (AUM daily) + **crystallize** (performance at redemption / period, vs high-water mark).
- **Reuses:** the **paper engine** (PRs #222/#223/#225) for paper-trading a strategy; the **historical-analysis backtest runner** (PR #227) for backtests; the **one-true-ledger + payouts** for subscriptions/redemptions + fee payout to the creator; and the **creator-token AUM/fee accounting** (this is the fund analogue of the token program — same NAV/units + fee-crystallization machinery).
- **Hard backend blockers:** a **strategy definition + rules engine** (evaluate rules against market data → target portfolio); a **pooled execution engine** that rebalances investor capital across the basket's symbols; **per-investor unit/NAV accounting** (subscribe/redeem at NAV, high-water mark per investor); **fee accrual + crystallization** (AUM daily + performance carry); **capacity + min-size enforcement**; and a **paper-vs-live** mode switch.
- **Open decisions (lock before build — biggest fork first):**
  1. **Pooled fund (one book, NAV units) vs copy/replication (each investor's own account mirrors the strategy).** Pooled = true fund accounting; copy = per-user positions, no commingling. This changes the entire backend.
  2. **Rule authoring:** predefined templates (weights + rebalance cadence + simple triggers) vs a full strategy DSL.
  3. **Performance-fee model:** high-water mark only, or + hurdle rate; crystallization cadence (on redemption vs quarterly).
  4. **Redemption:** instant-at-NAV vs notice/lockup/gates (illiquid-basket protection).
- **Frontend (buildable now, degrade-on-404):** a **strategy builder** (rules/weights + the min-size/capacity/redemption/fee params), a **paper-run + backtest** view (reusing paper + `/analysis`), a **strategy marketplace/browse** (with track record, AUM, fees, capacity-remaining), and an **investor position** view (units, NAV, fees paid, redeem). The trade/paper/analysis surfaces already exist to plug into.
...
## Added 2026-08-21 (pm) — creator-tokens epic SHIPPED + newly-surfaced backlog

### ✅✅ Creator-tokens epic — COMPLETE + FULLY DEPLOYED TO PROD
The **entire** "Tokenized creator revenue-share contracts" program (§B–I spec, PR #228 frontend) is now **built, tested, and live on prod** (both the exchange `i-08f937fc705ebea75` and the remote ME `.141`, main `8071921`). Every §D endpoint and every locked decision is implemented and prod-verified with exact money math. This closes the single largest backend-blocked item in this doc.
- ✅ **Mint + cap table** — `POST /me/tokens` ($100/10000¢ fee via one-true-ledger before mint, unique ticker, 100%→creator, idempotent) + `GET /me/tokens[/market|/{id}|/captable]`. Single DDB table `tlc_tokens` (single-table design). *(P1)*
- ✅ **Runtime engine symbol minting** — the "hard part / no edge-only workaround" (§C1) is **solved**: `order_manager_t::add_symbol()` stands up a new tradeable symbol on the LIVE engine (realloc-hazard neutralized via `reserve()`), reachable cross-box via additive wire ops `ME_OP_CREATE_SYMBOL`/`ME_OP_LIST_SYMBOLS` (no proto bump). *(P2·Inc1)*
- ✅ **§G single-clearing-price IPO batch auction** — `list → auction/bid (escrow) → auction/clear` with uniform clearing price, marginal pro-rata, over-escrow/loser refunds, cap-table transfer creator→winners, proceeds credit, `me_create_symbol` seeds the continuous book, `status:listed`. Prod-verified on an oversubscribed 2-bidder auction. *(P2·Inc3)*
- ✅ **§E revenue routing + §F upkeep** — `distribute → per-holder ACCR# accrual (snapshot) → revenue/claim`; `upkeep` (threshold $100, holder-pro-rata shortfall) + `upkeep/pay` + `upkeep/assess` (freeze-not-delist, reversible). Prod-verified. *(P2·Inc4)*
- ✅ **`instrument_class` in the picker** — `/md/symbols` now merges live engine symbols; listed tokens surface as `instrument_class:"creator_token"` (the §287 picker ask, for tokens). *(Inc2)*

### 📋 NEW backlog surfaced by completing the token epic (4 follow-ups — in progress)
Hardening items the shipped epic revealed. All four edit the same edge TU so they ship sequentially (recommended order below):
- [ ] **Engine halt-on-freeze** *(smallest)* — `upkeep/assess` freeze sets DDB `status:frozen` only; the ME book keeps trading. Halt the book on freeze / reopen on full payment by reusing the **existing** admin halt/reopen wire path (`me_admin_async` cmd 1=halt, 2→3=reopen) — no new wire op. §F says freeze must "halt the book"; today it doesn't.
- [ ] **Real per-token upkeep `fees_generated`** — `GET /me/tokens/{id}/upkeep` hardcodes `fees_generated=0` (so every token shows the full $100 shortfall). Feed it the token symbol's real monthly trading fees via an edge-side rollup off the fills feed (`me_fills` carries per-fill `fee`+`symbolid` for both maker & taker). Requires `TLC_ME_SPOT_FEES` on the ME for non-zero output.
- [ ] **Secondary-fill cap-table sync** — after listing, tokens trade on the continuous book and change hands **in the engine**, but the `H#` cap table is only written at mint + auction-clear, so it drifts. Add an idempotent background reconciler that applies per-sub engine fills (buyer qty+ / seller qty−) to `H#` — **token leg only, never money** (the engine already settles cash). The public trade tape is anonymous, so it must iterate subs via `me_fills` + a durable trader registry to cover offline sellers.
- [ ] **Auto revenue-skim hook** *(largest, feature-flagged)* — §E's "trigger" is currently the issuer-only `POST /distribute`; make it **automatic**: when creator content revenue lands in the ledger (subscription cycle / tip / unlock), skim `revenue_share_bps` of the net into the token's ACCR# accrual. Wire at the ledger-credit sites; ship behind a flag (it reduces creator payout). **Depends on** the cap-table-sync above (skimming onto a stale cap table pays the wrong holders).

### 📋 NEW backlog surfaced (token secondary-market gaps — not in the original spec)
- [ ] **Seed engine spot-config/balances for token symbols on listing** — auction-clear mints the symbol + seeds the book but does **not** call `me_spot_config`/`me_spot_deposit` for it, so it's unconfirmed whether the ME enforces token spot balances or permits **naked selling** (selling tokens you don't hold). If unenforced, the engine can't be trusted as the ownership source for the cap-table reconciler. Decide: spot-enforce token symbols (seed each holder's engine spot balance from the cap table at listing) vs. keep the DDB cap table authoritative and reconcile.
- [ ] **Initial secondary-market liquidity** — a freshly-listed token has an **empty continuous book** right after the IPO clears (holders must post orders). Consider a listing option to auto-seed a two-sided quote around the clearing price (issuer-provided or a light AMM/market-maker), so the token is immediately tradeable, not a dead book.
- [ ] **Token lifecycle: permanent delist / wind-down** — freeze is reversible, but there is no creator-exit / buyback / permanent-delist path (distinct from the deliberately-rejected non-payment delist). Nice-to-have for a creator who wants to retire a token (halt + final distribution + close).
- [ ] **Platform cut on distributions/proceeds** — revenue distributions currently flow 100% to holders and auction proceeds 100% to the creator (minus the $100 fee). If the platform should take a spread on distributions or a listing fee on proceeds, that's an unmodeled seam (§G4 mentions "minus any platform listing fee" — not implemented).

### 🎯 Next big backend programs (specced, shipped frontends, now unblocked)
With tokens done, the reusable primitives it built (§G uniform-clearing auction, escrow via `bill_wallet_delta`, ledger settlement, runtime symbol minting) **de-risk the two remaining shipped-but-unbuilt epics**:
- [ ] **Margin distress / pre-emptive bailout auctions** (PR #229 spec above) — the §G clearing algo, escrow, and refund machinery are now **built and reusable** for the bailout auction (target = `capital_needed` instead of offered qty). The genuinely-new work is the **distress read** (`GET /me/margin/distress`) + **position co-ownership** in the position ledger. Recommended next epic.
- [ ] **Multi-protocol trading/custody credentials** (PR #230 spec above) — independent of tokens; the unified gateway already spans REST/WS/FIX/binary. New work is the key model spanning 4 channels + FIX/binary credential provisioning + `GET /me/gateway/endpoints`.

### 🔗 NEW cross-cutting primitive (unblocks THREE things)
- [ ] **Shared realized-volatility signal** — a rolling stddev-of-returns over engine marks per symbol. It is independently required by **(a)** the per-coin conversion-fee model (§177 `k_var·vol`), **(b)** the bailout distress band (§296 `danger_bps = clamp(k×volatility_bps, ...)`), and **(c)** would improve token upkeep/fee estimation. Build **once** as an engine/edge primitive (with a `me_marks` mark-history feed) and three surfaces light up. Promotes the `me_marks` oracle (§178) from "hard dependency of pay-any-coin" to "shared dependency of pay-any-coin + bailouts + distress."

### Still-open from prior sections (unchanged, re-surfaced for priority)
- [ ] **`GET /md/history/{symbol}` paginated OHLCV** (§A) — the analysis workbench (PR #227) is still blocked on this; it's the cheapest high-value item (no engine change if candles are persisted).
- [ ] **Prediction-markets client surface** — engine core + `ME_OP_PM_*` exist; still need the REST/edge create/resolve/state + trader PM order path.
- [ ] **Fees/payments follow-ons** — per-coin conversion-fee model (needs the shared vol signal above), `me_marks` oracle, FX revenue accounting, trading-fee tiering, fiat USD on-ramp.
