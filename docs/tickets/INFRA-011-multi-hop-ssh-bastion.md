...
**File**: `tests/test_multi_hop_ssh.py`

Mock external dependencies with `moto` (DynamoDB) and `unittest.mock`. All tests run without the dev stack.

  - `test_create_bastion_chain`
  - `test_validate_chain_hops`
  - `test_resolve_chain_to_host_records`
  - `test_connect_through_single_bastion`
  - `test_connect_through_double_bastion`
  - `test_chain_fails_on_unreachable_hop`

### Integration Tests

  - Multi-hop connection creates nested Paramiko SSH tunnels
  - Each hop decrypts its SSH key from ssh_keys table
  - Session recording captures final-hop terminal output

### E2E Tests (Playwright)

**File**: `frontend/e2e/multi-hop-ssh.spec.ts`
**Test count**: 10
...
- **DDB seeds**: Seed `remote_hosts (bastion chain config)` table with test records in `beforeAll`
- **Test users**: Alice (USER), Bob (USER), Root (ROOT), Charlie (ADMIN) from `e2e_admin_session_setup.py`
- **Cleanup**: Tests use unique timestamps/IDs per run to avoid cross-run interference

### CI/Pipeline Considerations

- **Feature flag**: `MULTI_HOP_SSH_ENABLED=true` must be set in test environment
...
| Ticket | Title | Why |
|--------|-------|-----|
| INFRA-001 | Host Inventory Management | Bastion chains reference hosts from inventory |
| INFRA-002 | SSH Key Manager | Keys used for each hop in chain |
...
- [ ] All 10 E2E tests pass with `npx playwright test multi-hop-ssh.spec.ts`
