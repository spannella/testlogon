...
**Test file**: `tests/test_ad_scheduling.py`
...
|---|---|
| `test_create_ad_scheduling` | Creates record with correct fields and generated ID |
| `test_create_ad_scheduling_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_scheduling_found` | Returns correct record by ID |
| `test_get_ad_scheduling_not_found` | Returns None for non-existent ID |
| `test_list_ad_scheduling` | Returns all records for the given scope/owner |
| `test_update_ad_scheduling` | Updates mutable fields and sets updated_at |
| `test_delete_ad_scheduling` | Removes record; subsequent get returns None |
| `test_ad_scheduling_owner_check` | Rejects operations from non-owner users |
| `test_ad_scheduling_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_scheduling_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-scheduling.spec.ts`
...
- **Feature flags**: `AD_DAYPARTING_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Campaign manager (date fields) | Pending | No |
| ADS-004 | Serving engine (schedule check) | Pending | No |
| ADS-007 | Budget pacing integration | Pending | No |
...
|--------|------------------------|
| ADS-017 | Schedule-aware optimization |
