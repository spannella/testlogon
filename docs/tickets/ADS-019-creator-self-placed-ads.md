...
|---|---|
| `test_create_self_promos` | Creates record with correct fields and generated ID |
| `test_create_self_promos_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_self_promos_found` | Returns correct record by ID |
| `test_get_self_promos_not_found` | Returns None for non-existent ID |
| `test_list_self_promos` | Returns all records for the given scope/owner |
| `test_update_self_promos` | Updates mutable fields and sets updated_at |
| `test_delete_self_promos` | Removes record; subsequent get returns None |
| `test_self_promos_owner_check` | Rejects operations from non-owner users |
| `test_self_promos_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_self_promos_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-self-placed.spec.ts`
...
| Ticket | What's Needed | Status | Can Overlap? |
|--------|--------------|--------|-------------|
| ADS-001 | Account structure | Pending | Yes (parallel dev) |
| ADS-010 | Creator ad control settings | Pending | No |
...
**Sequential (after ADS-010)**


- Must merge after: ADS-001, ADS-010
...
- [ ] Frontend route(s) added to `App.tsx`: `/settings/self-ads`
