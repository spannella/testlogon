...
**Test file**: `tests/test_ad_serving.py`
...
|---|---|
| `test_create_ad_serving` | Creates record with correct fields and generated ID |
| `test_create_ad_serving_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_serving_found` | Returns correct record by ID |
| `test_get_ad_serving_not_found` | Returns None for non-existent ID |
| `test_list_ad_serving` | Returns all records for the given scope/owner |
| `test_update_ad_serving` | Updates mutable fields and sets updated_at |
| `test_delete_ad_serving` | Removes record; subsequent get returns None |
| `test_ad_serving_owner_check` | Rejects operations from non-owner users |
| `test_ad_serving_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_serving_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-serving.spec.ts`
...
|-------|--------------|-------|
| `AdFrequencyCaps` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |

### CI/Pipeline


- **Feature flags**: `AD_SERVING_ENABLED`, `AD_FREQUENCY_CAPPING_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Active campaigns with status checks | Pending | No |
| ADS-002 | Approved creatives for serving | Pending | No |
| ADS-003 | Targeting evaluation + creator prefs | Pending | No |
...
|--------|------------------------|
| ADS-005 | `serve_ad(surface="newsfeed")` |
| ADS-006 | `serve_ad(surface="broadcast")` |
| ADS-007 | `track_ad_event()` triggers billing |
| ADS-008 | Impression/click data for analytics |
| ADS-009 | User pref checks during serving |
| ADS-014 | Fraud detection on events |
| ADS-015 | Affiliate/promo on serve response |

### Merge Strategy


**Sequential (after ADS-001, ADS-002, ADS-003)**


- Must merge after: ADS-001, ADS-002, ADS-003
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AdFrequencyCaps`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `ad_frequency_caps_table_name`
