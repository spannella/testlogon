...
**Test file**: `tests/test_ad_billing.py`
...
|---|---|
| `test_create_ad_billing` | Creates record with correct fields and generated ID |
| `test_create_ad_billing_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_billing_found` | Returns correct record by ID |
| `test_get_ad_billing_not_found` | Returns None for non-existent ID |
| `test_list_ad_billing` | Returns all records for the given scope/owner |
| `test_update_ad_billing` | Updates mutable fields and sets updated_at |
| `test_delete_ad_billing` | Removes record; subsequent get returns None |
| `test_ad_billing_owner_check` | Rejects operations from non-owner users |
| `test_ad_billing_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_billing_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-billing.spec.ts`
...
|-------|--------------|-------|
| `AdBilling` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |

### CI/Pipeline


- **Feature flags**: `AD_BILLING_ENABLED`, `AD_BILLING_REVENUE_SPLIT_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Ad accounts (balance, spend fields) | Pending | No |
| ADS-004 | Ad tracking events trigger charges | Pending | No |
| Billing infra | `billing_shared.py` | Implemented | N/A |
| Alerts | `alerts.py` | Implemented | N/A |
...
|--------|------------------------|
| ADS-008 | Spending data for analytics |
| ADS-012 | Wallet deduction for boosting |
| ADS-015 | ROAS calculation |

### Merge Strategy


**Sequential (after ADS-001, ADS-004)**


- Must merge after: ADS-001, ADS-004
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AdBilling`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `ad_billing_table_name`
- [ ] `.env.local` updated with any new environment variables
- [ ] Router registered in `app/main.py` (from `app/routers/ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/ads/billing`
