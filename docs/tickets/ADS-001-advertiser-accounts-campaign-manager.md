...
**Test file**: `tests/test_ad_accounts.py`
...
|---|---|
| `test_create_ad_accounts` | Creates record with correct fields and generated ID |
| `test_create_ad_accounts_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_accounts_found` | Returns correct record by ID |
| `test_get_ad_accounts_not_found` | Returns None for non-existent ID |
| `test_list_ad_accounts` | Returns all records for the given scope/owner |
| `test_update_ad_accounts` | Updates mutable fields and sets updated_at |
| `test_delete_ad_accounts` | Removes record; subsequent get returns None |
| `test_ad_accounts_owner_check` | Rejects operations from non-owner users |
| `test_ad_accounts_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_accounts_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-accounts-campaigns.spec.ts`
...
|-------|--------------|-------|
| `AdAccounts` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |
| `AdCampaigns` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |
...
|--------|--------------|--------|-------------|
| Billing ledger | `billing_shared.py` | Implemented | N/A |
| Admin auth | `require_admin_or_root` in `app/auth/policy.py` | Implemented | N/A |
...
|--------|------------------------|
| ADS-002 | Campaign hierarchy for creative attachment |
| ADS-003 | Campaign hierarchy for targeting rules |
| ADS-004 | Account + campaign status for serving |
| ADS-007 | Account balance + budget for billing |
| ADS-008 | Campaign hierarchy for analytics |
| ADS-011 | Campaign CRUD for API |
| ADS-012 | Campaign creation for boosting |
| ADS-016 | Campaign scheduling for dayparting |
| ADS-017 | Campaign structure for optimization |
| ADS-018 | Account/campaign for admin dashboard |
| ADS-019 | Account structure for self-placed ads |
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AdAccounts`, `AdCampaigns`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `ad_accounts_table_name`, `ad_campaigns_table_name`
- [ ] `.env.local` updated with any new environment variables
- [ ] Router registered in `app/main.py` (from `app/routers/ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/ads/dashboard, /ads/campaigns`
