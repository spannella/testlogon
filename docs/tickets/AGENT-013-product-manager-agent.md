...
|---|---|
| `test_create_product_manager_agent` | Creates record with correct fields and generated ID |
| `test_create_product_manager_agent_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_product_manager_agent_found` | Returns correct record by ID |
| `test_get_product_manager_agent_not_found` | Returns None for non-existent ID |
| `test_list_product_manager_agent` | Returns all records for the given scope/owner |
| `test_update_product_manager_agent` | Updates mutable fields and sets updated_at |
| `test_delete_product_manager_agent` | Removes record; subsequent get returns None |
| `test_product_manager_agent_owner_check` | Rejects operations from non-owner users |
| `test_product_manager_agent_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_product_manager_agent_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-product-manager.spec.ts`
...
- [ ] Frontend route(s) added to `App.tsx`: `/agents/product-manager`
