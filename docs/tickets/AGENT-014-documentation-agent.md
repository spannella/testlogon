...
| Existing docs | `docs/` directory | — | `architecture.md`, `dynamodb.md`, `local-dev-stack.md`, `file-reference.md`, `CLAUDE.md` all confirmed to exist |
| OpenAPI spec | Backend `/openapi.json` | — | Auto-generated by FastAPI; available at runtime |
...
| Router registration | `app/main.py` | 297-465 | No `agent_docs_router` registered yet |
| `agent_docs.py` service | `app/services/` | — | Does NOT exist yet — new implementation in this ticket |
| `agent_docs.py` router | `app/routers/` | — | Does NOT exist yet — new implementation in this ticket |
...
**Test file**: `tests/test_documentation_agent.py`
...
|---|---|
| `test_create_documentation_agent` | Creates record with correct fields and generated ID |
| `test_create_documentation_agent_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_documentation_agent_found` | Returns correct record by ID |
| `test_get_documentation_agent_not_found` | Returns None for non-existent ID |
| `test_list_documentation_agent` | Returns all records for the given scope/owner |
| `test_update_documentation_agent` | Updates mutable fields and sets updated_at |
| `test_delete_documentation_agent` | Removes record; subsequent get returns None |
| `test_documentation_agent_owner_check` | Rejects operations from non-owner users |
| `test_documentation_agent_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_documentation_agent_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-docs.spec.ts`
...
- [ ] Frontend route(s) added to `App.tsx`: `/agents/docs`
