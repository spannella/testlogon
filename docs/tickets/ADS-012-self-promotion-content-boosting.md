...
**Test file**: `tests/test_content_boost.py`
...
|---|---|
| `test_create_content_boost` | Creates record with correct fields and generated ID |
| `test_create_content_boost_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_content_boost_found` | Returns correct record by ID |
| `test_get_content_boost_not_found` | Returns None for non-existent ID |
| `test_list_content_boost` | Returns all records for the given scope/owner |
| `test_update_content_boost` | Updates mutable fields and sets updated_at |
| `test_delete_content_boost` | Removes record; subsequent get returns None |
| `test_content_boost_owner_check` | Rejects operations from non-owner users |
| `test_content_boost_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_content_boost_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-boost.spec.ts`
...
- **Feature flags**: `CONTENT_BOOSTING_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Campaign creation for boosts | Pending | No |
| ADS-005 | Sponsored post rendering | Pending | No |
| ADS-007 | Wallet deduction | Pending | No |
...
**Sequential (after ADS-005, ADS-007)**


- Must merge after: ADS-001, ADS-005, ADS-007
