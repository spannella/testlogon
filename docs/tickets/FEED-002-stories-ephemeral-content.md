...
**File**: `tests/test_stories.py`

| # | Test Function | Description | Mocks |
|---|--------------|-------------|-------|
| 1 | `test_feed_002_create_basic` | Core creation logic succeeds with valid inputs | moto DDB |
| 2 | `test_feed_002_validation_rejects_invalid` | 400/422 for invalid inputs | moto DDB |
| 3 | `test_feed_002_pagination` | Cursor-based pagination returns correct pages | moto DDB |
| 4 | `test_feed_002_auth_required` | 401 for unauthenticated requests | moto DDB |
| 5 | `test_feed_002_forbidden_wrong_user` | 403 when non-owner accesses restricted resource | moto DDB |
| 6 | `test_feed_002_not_found` | 404 for non-existent resource | moto DDB |
| 7 | `test_feed_002_duplicate_rejected` | 409 for duplicate creation | moto DDB |
| 8 | `test_feed_002_feature_flag_off` | Feature disabled returns 404 when flag is off | moto DDB |
...
**File**: `frontend/e2e/stories.spec.ts`
...
- Feature flag: `STORIES_ENABLED` in `.env.local`

### CI/Pipeline

- Feature flag: `STORIES_ENABLED` must be enabled for tests to run
...
- [ ] Feature flag `STORIES_ENABLED` added to `.env.local.example`
- [ ] All E2E tests pass
