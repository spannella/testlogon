...
**Test file**: `tests/test_sponsorship_deals.py`
...
|---|---|
| `test_create_sponsorship_deals` | Creates record with correct fields and generated ID |
| `test_create_sponsorship_deals_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_sponsorship_deals_found` | Returns correct record by ID |
| `test_get_sponsorship_deals_not_found` | Returns None for non-existent ID |
| `test_list_sponsorship_deals` | Returns all records for the given scope/owner |
| `test_update_sponsorship_deals` | Updates mutable fields and sets updated_at |
| `test_delete_sponsorship_deals` | Removes record; subsequent get returns None |
| `test_sponsorship_deals_owner_check` | Rejects operations from non-owner users |
| `test_sponsorship_deals_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_sponsorship_deals_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-partnerships.spec.ts`
...
|-------|--------------|-------|
| `SponsorshipDeals` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |

### CI/Pipeline


- **Feature flags**: `SPONSORSHIP_DEALS_ENABLED` must be `true` in `.env.local`
...
| ADS-007 | Billing for milestone payments | Pending | No |
| ADS-008 | Analytics for campaign perf | Pending | No |
...
- Must merge after: ADS-001, ADS-007, ADS-008
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `SponsorshipDeals`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `sponsorship_deals_table_name`
- [ ] `.env.local` updated with any new environment variables
- [ ] Router registered in `app/main.py` (from `app/routers/ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/ads/partnerships`
