...
**Test file**: `tests/test_agent_orchestrator.py`
...
|---|---|
| `test_create_agent_orchestrator` | Creates record with correct fields and generated ID |
| `test_create_agent_orchestrator_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_agent_orchestrator_found` | Returns correct record by ID |
| `test_get_agent_orchestrator_not_found` | Returns None for non-existent ID |
| `test_list_agent_orchestrator` | Returns all records for the given scope/owner |
| `test_update_agent_orchestrator` | Updates mutable fields and sets updated_at |
| `test_delete_agent_orchestrator` | Removes record; subsequent get returns None |
| `test_agent_orchestrator_owner_check` | Rejects operations from non-owner users |
| `test_agent_orchestrator_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_agent_orchestrator_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-framework.spec.ts`
...
|-------|--------------|-------|
| `AgentTasks` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |
...
| Ticket system | `app/services/tickets.py` | Implemented | N/A |
...
|--------|------------------------|
| AGENT-004 | Agent lifecycle for fleet UI |
| AGENT-005 | Agent context for memory |
| AGENT-006 | Agent status for monitoring |
| AGENT-007 | Agent lifecycle for PR/ticket |
| AGENT-008 through AGENT-018 | Agent framework for all specialized agents |

### Merge Strategy


**Sequential (after AGENT-002)**


- Must merge after: AGENT-002
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AgentTasks`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `agent_tasks_table_name`
