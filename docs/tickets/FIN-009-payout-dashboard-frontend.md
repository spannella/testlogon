...
**File**: `tests/test_payout_dashboard.py`

| # | Test Function | Description | Mocks |
|---|--------------|-------------|-------|
| 1 | `test_fin_009_create_basic` | Core creation logic succeeds with valid inputs | moto DDB |
| 2 | `test_fin_009_validation_rejects_invalid` | 400/422 for invalid inputs | moto DDB |
| 3 | `test_fin_009_pagination` | Cursor-based pagination returns correct pages | moto DDB |
| 4 | `test_fin_009_auth_required` | 401 for unauthenticated requests | moto DDB |
| 5 | `test_fin_009_forbidden_wrong_user` | 403 when non-owner accesses restricted resource | moto DDB |
| 6 | `test_fin_009_not_found` | 404 for non-existent resource | moto DDB |
| 7 | `test_fin_009_duplicate_rejected` | 409 for duplicate creation | moto DDB |
| 8 | `test_fin_009_feature_flag_off` | Feature disabled returns 404 when flag is off | moto DDB |
...
**File**: `frontend/e2e/payout-dashboard.spec.ts`
...
- Feature flag: `None (frontend-only enhancement)` in `.env.local`

### CI/Pipeline

- Feature flag: `None (frontend-only enhancement)` must be enabled for tests to run
...
- [ ] Feature flag `None (frontend-only enhancement)` added to `.env.local.example`
- [ ] All E2E tests pass
