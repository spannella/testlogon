...
**Test file**: `tests/test_ad_creatives.py`
...
|---|---|
| `test_create_ad_creatives` | Creates record with correct fields and generated ID |
| `test_create_ad_creatives_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_creatives_found` | Returns correct record by ID |
| `test_get_ad_creatives_not_found` | Returns None for non-existent ID |
| `test_list_ad_creatives` | Returns all records for the given scope/owner |
| `test_update_ad_creatives` | Updates mutable fields and sets updated_at |
| `test_delete_ad_creatives` | Removes record; subsequent get returns None |
| `test_ad_creatives_owner_check` | Rejects operations from non-owner users |
| `test_ad_creatives_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_creatives_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-creatives.spec.ts`
...
|-------|--------------|-------|
| `AdCreatives` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |

### CI/Pipeline


- **Feature flags**: `AD_CREATIVES_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Campaign hierarchy (ad_campaigns table) | Pending | Yes (parallel dev) |
...
|--------|------------------------|
| ADS-004 | Approved creatives for serving |
| ADS-005 | Native post creatives for feed |
| ADS-006 | Video creatives for broadcast |
| ADS-008 | Creative-level analytics |
| ADS-015 | Creative + affiliate integration |
| ADS-017 | Creative rotation optimization |

### Merge Strategy


**Sequential (after ADS-001)**


- Must merge after: ADS-001
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AdCreatives`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `ad_creatives_table_name`
- [ ] `.env.local` updated with any new environment variables
- [ ] Router registered in `app/main.py` (from `app/routers/ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/ads/campaigns (creative editor)`
