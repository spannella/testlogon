...
|---|---|
| `test_create_agent_memory` | Creates record with correct fields and generated ID |
| `test_create_agent_memory_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_agent_memory_found` | Returns correct record by ID |
| `test_get_agent_memory_not_found` | Returns None for non-existent ID |
| `test_list_agent_memory` | Returns all records for the given scope/owner |
| `test_update_agent_memory` | Updates mutable fields and sets updated_at |
| `test_delete_agent_memory` | Removes record; subsequent get returns None |
| `test_agent_memory_owner_check` | Rejects operations from non-owner users |
| `test_agent_memory_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_agent_memory_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-memory.spec.ts`
...
|-------|--------------|-------|
| `AgentMemory` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |
...
|--------|--------------|--------|-------------|
| AGENT-002 | Worker provisioning | Pending | No |
...
|--------|------------------------|
| AGENT-008 through AGENT-018 | Context injection for specialized agents |
...
- Must merge after: AGENT-002, AGENT-003
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AgentMemory`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `agent_memory_table_name`
