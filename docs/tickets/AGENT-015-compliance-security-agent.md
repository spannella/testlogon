...
**Test file**: `tests/test_compliance_agent.py`
...
|---|---|
| `test_create_compliance_agent` | Creates record with correct fields and generated ID |
| `test_create_compliance_agent_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_compliance_agent_found` | Returns correct record by ID |
| `test_get_compliance_agent_not_found` | Returns None for non-existent ID |
| `test_list_compliance_agent` | Returns all records for the given scope/owner |
| `test_update_compliance_agent` | Updates mutable fields and sets updated_at |
| `test_delete_compliance_agent` | Removes record; subsequent get returns None |
| `test_compliance_agent_owner_check` | Rejects operations from non-owner users |
| `test_compliance_agent_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_compliance_agent_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-compliance.spec.ts`
...
- [ ] Frontend route(s) added to `App.tsx`: `/agents/compliance`
