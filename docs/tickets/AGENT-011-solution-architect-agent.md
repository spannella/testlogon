...
**Test file**: `tests/test_architect_agent.py`
...
|---|---|
| `test_create_architect_agent` | Creates record with correct fields and generated ID |
| `test_create_architect_agent_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_architect_agent_found` | Returns correct record by ID |
| `test_get_architect_agent_not_found` | Returns None for non-existent ID |
| `test_list_architect_agent` | Returns all records for the given scope/owner |
| `test_update_architect_agent` | Updates mutable fields and sets updated_at |
| `test_delete_architect_agent` | Removes record; subsequent get returns None |
| `test_architect_agent_owner_check` | Rejects operations from non-owner users |
| `test_architect_agent_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_architect_agent_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-architect.spec.ts`
...
|--------|------------------------|
| AGENT-012 | Architect agent for design review |
...
- [ ] Frontend route(s) added to `App.tsx`: `/agents/architect`
