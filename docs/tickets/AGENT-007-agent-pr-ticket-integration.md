...
**Test file**: `tests/test_agent_pr_ticket.py`
...
|---|---|
| `test_create_agent_pr_ticket` | Creates record with correct fields and generated ID |
| `test_create_agent_pr_ticket_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_agent_pr_ticket_found` | Returns correct record by ID |
| `test_get_agent_pr_ticket_not_found` | Returns None for non-existent ID |
| `test_list_agent_pr_ticket` | Returns all records for the given scope/owner |
| `test_update_agent_pr_ticket` | Updates mutable fields and sets updated_at |
| `test_delete_agent_pr_ticket` | Removes record; subsequent get returns None |
| `test_agent_pr_ticket_owner_check` | Rejects operations from non-owner users |
| `test_agent_pr_ticket_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_agent_pr_ticket_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/agent-pr-ticket.spec.ts`
...
| AGENT-006 | Terminal monitoring data | Pending | No |
...
|--------|------------------------|
| AGENT-008 | PR integration for coder agent |
| AGENT-009 | Ticket integration for QA agent |
| AGENT-013 through AGENT-018 | PR/ticket integration for specialized agents |
...
- Must merge after: AGENT-003, AGENT-006
