...
**File**: `tests/test_exchange_rates.py`

| # | Test Function | Description | Mocks |
|---|--------------|-------------|-------|
| 1 | `test_fin_005_create_basic` | Core creation logic succeeds with valid inputs | moto DDB |
| 2 | `test_fin_005_validation_rejects_invalid` | 400/422 for invalid inputs | moto DDB |
| 3 | `test_fin_005_pagination` | Cursor-based pagination returns correct pages | moto DDB |
| 4 | `test_fin_005_auth_required` | 401 for unauthenticated requests | moto DDB |
| 5 | `test_fin_005_forbidden_wrong_user` | 403 when non-owner accesses restricted resource | moto DDB |
| 6 | `test_fin_005_not_found` | 404 for non-existent resource | moto DDB |
| 7 | `test_fin_005_duplicate_rejected` | 409 for duplicate creation | moto DDB |
| 8 | `test_fin_005_feature_flag_off` | Feature disabled returns 404 when flag is off | moto DDB |
...
- Feature flag: `MULTI_CURRENCY_ENABLED` in `.env.local`

### CI/Pipeline

- Feature flag: `MULTI_CURRENCY_ENABLED` must be enabled for tests to run
...
**Feature-flag-gated** -- Changes are additive (new service files, new router, new frontend pages). Shared infrastructure files (`main.py`, `settings.py`, `tables.py`, `local-ddb-init.py`) receive only additive modifications.
...
- [ ] Feature flag `MULTI_CURRENCY_ENABLED` added to `.env.local.example`
- [ ] All E2E tests pass
