...
|---|-----------|-------|-------------|-----------|-----------|
| 1 | Verify follow exists | social | PK=`FOLLOW#{follower}`, SK=`{following}` | None | Per snooze/unsnooze |
| 2 | Set snooze | social | PK=`FOLLOW#{follower}`, SK=`{following}` | `attribute_exists(pk)` | Per snooze |
| 3 | Clear snooze | social | PK=`FOLLOW#{follower}`, SK=`{following}` | `attribute_exists(pk)` | Per unsnooze |
| 4 | List followings | social | PK=`FOLLOW#{follower}`, SK begins_with `USER#` | None | Per feed request |
| 5 | Filter snoozed | In-memory | N/A | `snoozed_until > now_ts()` | Per feed request |
| 6 | List snoozed only | social + filter | PK=`FOLLOW#{follower}` | `snoozed_until > :now` | Per management page |
...
```json
{
  "pk": "FOLLOW#alice@test.local",
  "sk": "USER#bob@test.local",
