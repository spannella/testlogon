...
**Test file**: `tests/test_creator_ad_controls.py`
...
|---|---|
| `test_create_creator_ad_controls` | Creates record with correct fields and generated ID |
| `test_create_creator_ad_controls_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_creator_ad_controls_found` | Returns correct record by ID |
| `test_get_creator_ad_controls_not_found` | Returns None for non-existent ID |
| `test_list_creator_ad_controls` | Returns all records for the given scope/owner |
| `test_update_creator_ad_controls` | Updates mutable fields and sets updated_at |
| `test_delete_creator_ad_controls` | Removes record; subsequent get returns None |
| `test_creator_ad_controls_owner_check` | Rejects operations from non-owner users |
| `test_creator_ad_controls_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_creator_ad_controls_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-creator-controls.spec.ts`
...
|--------|--------------|--------|-------------|
| ADS-003 | Creator ad preferences (base) | Pending | No |
| ADS-004 | Ad serving engine | Pending | No |
| ADS-007 | Revenue split for earnings | Pending | No |
...
|--------|------------------------|
| ADS-019 | Creator ad controls for self-placed ads |

### Merge Strategy


**Sequential (after ADS-003, ADS-007)**


- Must merge after: ADS-003, ADS-004, ADS-007
...
- [ ] Frontend route(s) added to `App.tsx`: `/settings/creator-ads`
