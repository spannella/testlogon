...
**Test file**: `tests/test_ad_analytics.py`
...
|---|---|
| `test_create_ad_analytics` | Creates record with correct fields and generated ID |
| `test_create_ad_analytics_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_ad_analytics_found` | Returns correct record by ID |
| `test_get_ad_analytics_not_found` | Returns None for non-existent ID |
| `test_list_ad_analytics` | Returns all records for the given scope/owner |
| `test_update_ad_analytics` | Updates mutable fields and sets updated_at |
| `test_delete_ad_analytics` | Removes record; subsequent get returns None |
| `test_ad_analytics_owner_check` | Rejects operations from non-owner users |
| `test_ad_analytics_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_ad_analytics_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-analytics.spec.ts`
...
|-------|--------------|-------|
| `AdAnalyticsRollups` | See DDB schema in technical design section | Created by `scripts/local-ddb-init.py` |

### CI/Pipeline


- **Feature flags**: `AD_ANALYTICS_ENABLED` must be `true` in `.env.local`
...
|--------|--------------|--------|-------------|
| ADS-001 | Account + campaign hierarchy | Pending | No |
| ADS-004 | Impression/click data | Pending | No |
| ADS-007 | Spending data from billing | Pending | No |
...
|--------|------------------------|
| ADS-011 | Analytics data for API reporting |
| ADS-017 | Analytics for optimization |
| ADS-018 | Platform-wide analytics for admin |

### Merge Strategy


**Sequential (after ADS-007)**
...
- [ ] DDB table(s) added to `scripts/local-ddb-init.py`: `AdAnalyticsRollups`
- [ ] Settings added to `app/core/settings.py` + `app/core/tables.py`: `ad_analytics_rollups_table_name`
- [ ] `.env.local` updated with any new environment variables
- [ ] Router registered in `app/main.py` (from `app/routers/ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/ads/analytics`
