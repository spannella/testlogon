...
|---|---|
| `test_create_admin_ads` | Creates record with correct fields and generated ID |
| `test_create_admin_ads_validation` | Rejects invalid input (missing required fields, out-of-range values) |
| `test_get_admin_ads_found` | Returns correct record by ID |
| `test_get_admin_ads_not_found` | Returns None for non-existent ID |
| `test_list_admin_ads` | Returns all records for the given scope/owner |
| `test_update_admin_ads` | Updates mutable fields and sets updated_at |
| `test_delete_admin_ads` | Removes record; subsequent get returns None |
| `test_admin_ads_owner_check` | Rejects operations from non-owner users |
| `test_admin_ads_admin_only` | Admin/root endpoints reject USER role with 403 |
| `test_admin_ads_concurrent_write` | Conditional update prevents stale overwrites |
...
**Test file**: `frontend/e2e/ads-admin.spec.ts`
...
| ADS-002 | Creatives (review queue) | Pending | No |
| ADS-004 | Serving (metrics) | Pending | No |
| ADS-007 | Billing (revenue data) | Pending | No |
| ADS-008 | Analytics (platform-wide) | Pending | No |
| ADS-014 | Fraud prevention metrics | Pending | No |
...
**Sequential (after ADS-014)**


- Must merge after: ADS-001, ADS-002, ADS-004, ADS-007, ADS-008, ADS-014
...
- [ ] Router registered in `app/main.py` (from `app/routers/admin_ads.py`)
- [ ] Frontend route(s) added to `App.tsx`: `/admin/ads`
