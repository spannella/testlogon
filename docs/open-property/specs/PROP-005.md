...
| **Work Orders FE** (gap analysis §C) | Work-order tab in `PropertyDetailPage` linking to ticket boards (`app/services/tickets.py` `_DEFAULT_BOARD_COLUMNS`/`_STATUS_TRANSITIONS`); property/unit FK fields on work-order rows per the WOV cluster (`docs/open-property/specs/WOV-001.md` — `maintenance_orders` table keyed on `PROPERTY#{property_id}` with additive `priority`/`unit_id`/`vendor_id`; WOV-005 ships the Kanban FE). WOV-001 is the property-scoped variant of the FXA-012 maintenance-order analogue, NOT FXA-012/013 itself (those are keyed on `asset_id`). |
...
| **Document linking** (gap analysis §C) | EVT-011 record-link fields attach files (via `filemanager.py`) to records via `linked_record_type`/`linked_record_id` (`docs/suitecrm/specs/EVT-011.md:82`). NOTE: EVT-011's `_LINKED_RECORD_TYPES` allowlist is currently `{contact, ticket, account, party}` (`:82`) — it does NOT yet include `property`/`unit`. Attaching files to a property/unit requires the PROP vertical to add those values to the EVT-011 allowlist when it ships (an additive EVT-011 change, not an existing feature). EVT-012 revisions for lease versions; surface as a "Documents" tab in `PropertyDetailPage`. |

---

## 12. Verification Log

Each assumption in the original spec was verified against the live codebase at repo root `/home/ubuntu/testlogon`. Status: **VERIFIED**, **CORRECTED**, or **FORWARD_DEP** (artifact defined by a sibling spec). No UNCONFIRMED/RISK rows remain.
...
| 35 | `EVT-011` attaches files to `property_id`/`unit_id` FKs | CORRECTED | EVT-011 (`docs/suitecrm/specs/EVT-011.md:82`) defines `linked_record_type`/`linked_record_id` with a **closed allowlist** `_LINKED_RECORD_TYPES = {contact, ticket, account, party}` — `property`/`unit` are NOT registered values; an unknown type → HTTP 400 (`:134`). §11.3 "Document linking" row corrected to state plainly that attaching files to a property/unit requires the PROP vertical to ADD `property`/`unit` to the EVT-011 allowlist (an additive EVT-011 change), not an existing EVT-011 feature. |
| 36 | `FXA-012`/`FXA-013` add property/unit FK fields to work-orders | FORWARD_DEP — WOV-001/002 | `docs/ofbiz/specs/FXA-012.md` keys maintenance orders on `asset_id` (fixed-asset FK), NOT `property_id`/`unit_id`. The property-scoped work-order entity is owned by the sibling WOV cluster: `docs/open-property/specs/WOV-001.md` §1 defines `app/services/maintenance_orders.py` with the `maintenance_orders` table keyed on `PROPERTY#{property_id}` and additive `priority`/`unit_id`/`vendor_id` fields (the property-scoped variant of the FXA-012 analogue); WOV-002 adds the lifecycle; WOV-005 ships the FE. §11.3 "Work Orders FE" row corrected to cite WOV-001 instead of FXA-012/013. |
...
**Summary**: 7 CORRECTED, 1 FORWARD_DEP (WOV-001/002), 32 VERIFIED. No UNCONFIRMED/RISK rows remain.
