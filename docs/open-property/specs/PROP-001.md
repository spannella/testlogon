...
| Work orders | Extend `app/services/tickets.py` (`_DEFAULT_BOARD_COLUMNS`, `_STATUS_TRANSITIONS`) with property/unit FK fields; add `priority`, `scheduled_for`, `cost` per FXA-012/013 spec |
...
| 28 | FXA-012 / FXA-013 specs exist; `scheduled_for`, `cost` (`cost_cents`) fields present | **VERIFIED** | `docs/ofbiz/specs/FXA-012.md` — `cost_cents` at `:14`, `scheduled_for` at `:116`; `priority` field is **absent** from FXA-012 (no mention) — RISK: the §11.4 claim "add `priority`, `scheduled_for`, `cost` per FXA-012/013 spec" is partially unverified for `priority` |
...
**Summary**: 1 correction applied (item 30 — deprecated `moto.mock_dynamodb()` API). 1 RISK flag (item 28 — `priority` field in FXA-012/013 not verified; `cost_cents` and `scheduled_for` confirmed). All other 31 claims verified exact against codebase.
