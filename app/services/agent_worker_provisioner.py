...
def _provision_worker_dev(
    ...
    try:
        ...
        _append_provision_step(user_id, worker_id, "tool_install", "done", f"{tool}@{tool_version}")
...
def list_workers(
    ...
    *,
    ...
    items = resp.get("Items", [])
    if status:
        items = [i for i in items if i.get("worker_status") == status]
    if agent_type:
        items = [i for i in items if i.get("agent_type") == agent_type]
    return [_item_to_worker(item) for item in items]
