...
def _reconcile_lost(incident: Dict[str, Any], charge: Dict[str, Any], payer_anchor: str, *, outcome: str, actor: str) -> Dict[str, Any]:
    ...
    if charge and charge.get("charge_type") and charge.get("charge_ref"):
        ...
        # DISP-040: feed the fraud engine -- a LOST/ACCEPTED chargeback is a real
        # abuse signal against the payer (increments the risk row + auto-flags +
        # optional auto-freeze). Idempotent per incident; best-effort.
        try:
            from app.services import dispute_fraud as DF
            payer_for_fraud = charge.get("payer_id") or incident.get("customer_id") or ""
            if payer_for_fraud and str(payer_for_fraud) != "unknown":
                DF.record_dispute_fraud_signal(
                    payer_id=str(payer_for_fraud),
                    amount_cents=_chargeback_amount_cents(incident),
                    tx_id=incident_id,
                    kind="chargeback",
                    signal_ref=f"cb:{incident_id}",
                    actor=actor,
                )
        except Exception:
            logger.exception("chargeback fraud-signal hook failed incident=%s", incident_id)
...
def on_incident_transition(incident: Dict[str, Any], *, target_status: str, source_event_type: str = "",
    ...
    try:
        if status == "opened":
            hold = hold_charge_on_open(incident, actor=actor)
            # DISP-042: converge tracks -> auto-moot any open USER dispute on the
            # same charge + cross-link, so only ONE track drives the rail.
            link = {}
            try:
                from app.services import dispute_fraud as DF
                link = DF.link_and_moot_on_processor_open(incident, actor=actor)
            except Exception:
