...
def populate_on_approval(
    *,
    ...
    currency: str = "USD",
    ship_to: Optional[Dict[str, Any]] = None,  # ECOMX-40: explicit checkout address
) -> List[Dict[str, Any]]:
    ...
    # ECOMX-40 (B3): prefer the address explicitly selected in the checkout step;
    # fall back to the buyer profile / primary saved address.
    ship_to = ship_to if (isinstance(ship_to, dict) and any(v for v in ship_to.values()))         else _resolve_ship_to(buyer, buyer_sub)
...
def _notify_buyer_shipped(row: Dict[str, Any]) -> None:
