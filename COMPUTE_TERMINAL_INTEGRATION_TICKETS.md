...
**✅ DONE (2026-06-09, commit 2048256c)** — ported the existing `BrowserSshPage` (xterm + addon-fit) from the `spannella/backoffice` fork into testlogon: route `remote/ssh` behind `isBrowserSshEnabled`, sidebar entry + `nav.browserSsh` locales, vite `/api/browser-ssh` `ws:true` proxy, and query-param deep-link prefill. Verified by `frontend/e2e/browser-ssh.spec.ts` (2/2) + clean vite build.

**ALSO DONE (2026-06-09, commit 730380f6):** **CTI-001** — the WS connect handler resolves a registered `host_id` → authoritative host/port/username from the owner's inventory before validation (`_resolve_host_id_into_payload` in `app/routers/browser_ssh_terminal.py`; host overridden anti-spoof, port/username filled when omitted; never raises). Test `tests/test_cti_001_host_id_resolution.py` (5/5). **CTI-007/008** — running EC2 (Linux) + K8s pods show an "Open terminal" button deep-linking to `/remote/ssh?host_id=…`. **CTI-002 DONE** (commit pending) — VNC `user:{host_id}` targets resolve from inventory. Remaining (optional): **CTI-005** (RDP transport).
