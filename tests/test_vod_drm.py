...
class TestKeyDerivation:
    ...
    def test_derives_16_byte_key(self):
        ...
        key = derive_content_key("asset-123")
    ...
    def test_deterministic_same_asset(self):
        ...
        key1 = derive_content_key("asset-abc")
        key2 = derive_content_key("asset-abc")
        assert key1 == key2

    def test_deterministic_same_asset_same_slot(self):
        from app.services.vod_drm_keys import derive_content_key

        key1 = derive_content_key("asset-xyz", key_slot=3)
        key2 = derive_content_key("asset-xyz", key_slot=3)
        assert key1 == key2

    def test_different_asset_produces_different_key(self):
        from app.services.vod_drm_keys import derive_content_key

        key1 = derive_content_key("asset-aaa")
        key2 = derive_content_key("asset-bbb")
        assert key1 != key2

    def test_different_slot_produces_different_key(self):
        from app.services.vod_drm_keys import derive_content_key

        key1 = derive_content_key("asset-123", key_slot=0)
        key2 = derive_content_key("asset-123", key_slot=1)
        assert key1 != key2
...
class TestKeyIdDerivation:
    ...
    def test_key_id_is_hex_string(self):
        ...
        key_id = derive_key_id("asset-123")
    ...
    def test_key_id_is_deterministic(self):
        ...
        id1 = derive_key_id("my-video")
        id2 = derive_key_id("my-video")
        assert id1 == id2

    def test_different_asset_different_key_id(self):
        from app.services.vod_drm_keys import derive_key_id

        id1 = derive_key_id("video-1")
        id2 = derive_key_id("video-2")
        assert id1 != id2

    def test_key_id_url_safe(self):
        from app.services.vod_drm_keys import derive_key_id

        key_id = derive_key_id("asset/with/special chars!")
...
class TestKeyUri:
    ...
    def test_key_uri_format(self):
        ...
        uri = get_key_uri("test-asset")
        key_id = derive_key_id("test-asset")
...
class TestValidateKeyId:
    ...
    def test_valid_key_id_matches(self):
        ...
        key_id = derive_key_id("asset-test")
        assert validate_key_id(key_id, "asset-test") is True

    def test_invalid_key_id_does_not_match(self):
        from app.services.vod_drm_keys import validate_key_id

        assert validate_key_id("deadbeef" * 4, "asset-test") is False
...
class TestKeyServerEndpoint:
    ...
    def test_returns_401_without_token(self, client):
        ...
        key_id = derive_key_id("drm-test-asset")
    ...
    def test_returns_403_with_wrong_asset_id(self, client, valid_token):
        ...
        key_id = derive_key_id("other-asset")
...
class TestDrmInfoEndpoint:
    ...
    def test_info_returns_drm_metadata(self, client):
        resp = client.get("/v1/vod/drm/info/my-video-asset")
        assert resp.status_code == 200
        data = resp.json()
...
class TestEncryptionParams:
    ...
    def test_prepare_encryption_params_creates_files(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            params = prepare_encryption_params("test-asset-enc", scratch_dir=Path(td))
    ...
    def test_keyinfo_file_format(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            params = prepare_encryption_params("asset-keyinfo", scratch_dir=Path(td))
            assert params is not None
    ...
    def test_ffmpeg_encryption_args(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            params = prepare_encryption_params("asset-ffmpeg", scratch_dir=Path(td))
            assert params is not None
    ...
    def test_prepare_returns_none_when_disabled(self):
        ...
        with patch("app.services.vod_encryption.is_drm_enabled", return_value=False):
            params = prepare_encryption_params("asset-disabled")
    ...
    def test_cleanup_removes_files(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            params = prepare_encryption_params("asset-cleanup", scratch_dir=Path(td))
            assert params is not None
            assert Path(params.key_file_path).exists()
            assert Path(params.keyinfo_file_path).exists()
...
class TestFfmpegPipelineIntegration:
    ...
    def test_build_args_with_encryption(self, _mock_ffmpeg):
        ...
        with tempfile.TemporaryDirectory() as td:
            params = prepare_encryption_params("pipeline-asset", scratch_dir=Path(td))
            assert params is not None
    ...
    def test_ext_x_key_tag_generation(self):
        ...
        tag = build_encrypted_manifest_ext_x_key("my-video-001")
