...
class TestHideMessageEndpoint(unittest.TestCase):
    def test_hide_message_for_me_upserts_hidden_state_and_is_idempotent(self):
        ...
        with (
            ...
            out1 = messaging.hide_message_for_me("c1", "m1", user_id="u1")
            out2 = messaging.hide_message_for_me("c1", "m1", user_id="u1")

        self.assertEqual(metric_mock.call_count, 2)
        self.assertEqual(archive_mock.call_count, 2)

        self.assertEqual(out1.action, "hidden")
        self.assertEqual(out2.action, "hidden")
        ...
        self.assertEqual(first_call["ExpressionAttributeValues"][":state"], "hidden")
        self.assertEqual(first_call["ExpressionAttributeValues"][":updated_at"], 1700000000)
        self.assertEqual(first_call["ExpressionAttributeValues"][":conversation_user"], "c1#u1")

    def test_hide_message_for_me_rejects_non_participant(self):
        fake_table = _FakeVisibilityTable()
        with (
            ...
            with self.assertRaises(HTTPException) as ctx:
                messaging.hide_message_for_me("c-forbidden", "m1", user_id="u1")

        self.assertEqual(ctx.exception.status_code, 403)
        self.assertEqual(len(fake_table.calls), 0)

    def test_hide_message_for_me_only_writes_callers_scope(self):
        fake_table = _FakeVisibilityTable()
    ...
    def test_hide_message_for_me_rejects_when_feature_disabled(self):
