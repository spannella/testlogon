...
# Public base URL for return/cancel URLs (your webapp)
PUBLIC_BASE_URL = os.environ.get("PUBLIC_BASE_URL", "http://localhost:8000").rstrip("/")

DEFAULT_MONTHLY_PRICE_CENTS = int(os.environ.get("DEFAULT_MONTHLY_PRICE_CENTS", "999"))  # $9.99
DEFAULT_CURRENCY = os.environ.get("DEFAULT_CURRENCY", "usd").lower()
...
PAYPAL_WEBHOOK_ID = os.environ.get("PAYPAL_WEBHOOK_ID")  # required to verify webhooks

# Optional: map your "plan_id" to PayPal plan IDs (comma-separated: monthly=...,pro=...)
# Example: PAYPAL_PLAN_MAP="monthly:P-123,pro:P-456"
PAYPAL_PLAN_MAP_RAW = os.environ.get("PAYPAL_PLAN_MAP", "")
PAYPAL_PLAN_MAP: Dict[str, str] = {}
if PAYPAL_PLAN_MAP_RAW.strip():
    for part in PAYPAL_PLAN_MAP_RAW.split(","):
        if ":" in part:
            k, v = part.split(":", 1)
            PAYPAL_PLAN_MAP[k.strip()] = v.strip()

ddb = boto3.resource("dynamodb", region_name=AWS_REGION)
tbl = ddb.Table(DDB_TABLE)

app = FastAPI(title="Billing Backend (Ledger + PayPal Vault/Orders/Subscriptions)")
...
def _origin_ip(req: Request) -> str:
...
def put_payment_record(
    user_id: str,
    external_id: str,     # order_id or capture_id (choose one and be consistent)
    amount_cents: int,
...
def paypal_create_setup_token(
    *,
    user_id: str,
    usage_type: str,     # "MERCHANT" is common; keep as provided by your integration
    pm_kind: str,        # "paypal" or "card"
    return_url: str,
    cancel_url: str,
    idempotency_key: str,
) -> Dict[str, Any]:
    """
    Creates a setup token and returns an approval URL.
    """
    access = paypal_oauth()
    ...
    # Minimal request; you can enrich with customer/experience context later.
    # PayPal Wallet:
    if pm_kind == "paypal":
        payment_source = {"paypal": {"experience_context": {"return_url": return_url, "cancel_url": cancel_url}}}
    # Card (direct API vaulting often pairs with JS SDK if you aren't PCI compliant; still leaving shape here):
    else:
    ...
    payload = {
        ...
        "custom_id": user_id,  # helps correlate
    }
...
def paypal_exchange_setup_for_payment_token(
    *,
    ...
    """
    Exchanges an approved setup token for a permanent payment token.
    """
    access = paypal_oauth()
...
def paypal_delete_payment_token(payment_token_id: str) -> None:
    ...
    if r.status_code not in (204, 200):
        # non-fatal; you can choose to hard-fail instead
...
def paypal_create_order(
    *,
    ...
    # Best-effort: if we have a vaulted payment token, provide it as payment_source.token.
    # If PayPal still requires payer action, you'll receive an approve link.
    if payment_token_id:
...
def mark_webhook_processed(dedupe_key: str) -> bool:
    ...
    try:
        ddb_put(
            {"pk": "PAYPAL_WEBHOOK", "sk": dedupe_key, "ts": now_ts(), "ttl": now_ts() + 60 * 60 * 24 * 7},
...
class PaymentMethodOut(BaseModel):
    ...
    priority: int
    pm_type: Optional[str] = None  # "paypal" | "card" | unknown
...
class SubscribeMonthlyIn(BaseModel):
    plan_id: str = "monthly"  # your internal name
    paypal_plan_id: Optional[str] = None  # override
    idempotency_key: Optional[str] = None
...
def index():
    return """<!doctype html>
<html><head><meta charset="utf-8"/><title>Billing (PayPal)</title></head>
<body style="font-family:system-ui;margin:24px;max-width:900px">
<h2>Billing (PayPal)</h2>
<p>This backend provides ledger + PayPal Vault (payment tokens) + Orders + Subscriptions.</p>
</body></html>"""
...
@app.post("/api/billing/payment-methods/paypal/setup-token")
...
@app.post("/api/billing/payment-methods/paypal/exchange-token")
def exchange_setup_token(body: ExchangeTokenIn, x_user_id: Optional[str] = Header(default=None)):
    ...
    ddb_put({
        ...
        "payment_token_id": payment_token_id,
        "label": body.label,
        "pm_type": (resp.get("payment_source") or {}).keys().__iter__().__next__() if isinstance(resp.get("payment_source"), dict) and resp.get("payment_source") else None,
...
def remove_payment_method(payment_token_id: str, x_user_id: Optional[str] = Header(default=None), delete_from_paypal: bool = False):
    user_id = require_user(x_user_id)
    pk = user_pk(user_id)
    sk = pm_sk(payment_token_id)
    if not ddb_get(pk, sk):
...
async def charge_once(body: OneTimeChargeIn, request: Request, x_user_id: Optional[str] = Header(default=None)):
    ...
    # create order (may return approve link, even with token)
    ...
    # Record payment as "pending" keyed by order_id so webhooks/capture can correlate
    if order_id:
        put_payment_record(
            user_id=user_id,
...
async def subscribe_monthly(body: SubscribeMonthlyIn, x_user_id: Optional[str] = Header(default=None)):
    ...
    # Create subscription (approval required); status becomes active via webhook.
...
@app.post("/api/billing/subscriptions/cancel")
...
async def paypal_webhook(req: Request):
    ...
    # - For Orders/Captures: try to read custom_id "user|LEDGER#..." from purchase_units[0].custom_id
    # - For subscriptions: use resource.custom_id if present
    user_id = None
    ledger_sk_hint = None
    ...
    # Order/capture shapes vary; attempt common paths
    pu = None
    if isinstance(resource, dict):
        # capture webhook often has resource.supplementary_data.related_ids.order_id
        pass

    # Try custom_id from purchase_units if available
    try:
    ...
    # --- Handle core events (extend as needed)
    # Orders/captures: settle pending credit
    if event_type in ("PAYMENT.CAPTURE.COMPLETED", "CHECKOUT.ORDER.APPROVED"):
        # Prefer to settle on CAPTURE.COMPLETED; APPROVED just means payer approved
        if event_type == "PAYMENT.CAPTURE.COMPLETED":
            # If you created a PAY#order_id record, you can look it up by related order id.
            order_id = None
            try:
            ...
            # If we can find an internal payment record by order_id, settle it
            if order_id:
                ...
                if pay and pay.get("ledger_sk"):
                    ...
                    # fallback: create settled credit
                    amount_cents = 0
                    try:
        ...
        # Conservative: add settled debit adjustment (user owes again)
        amount_cents = 0
        try:
            amt = resource.get("amount") or {}
            amount_cents = int(Decimal(str(amt.get("value"))) * 100)
    ...
    # Subscriptions lifecycle
        ...
        sub_id = str(resource.get("id") or "")
        if sub_id:
            upsert_subscription(user_id, sub_id, status=event_type.split(".")[-1].lower(), plan_id="monthly", raw={"event": event})
