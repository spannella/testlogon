...
# How long to keep view receipts / edit history (TTL optional)
...
tbl_convos = dynamodb.Table(DDB_CONVERSATIONS)
tbl_parts  = dynamodb.Table(DDB_PARTICIPANTS)
tbl_msgs   = dynamodb.Table(DDB_MESSAGES)
tbl_events = dynamodb.Table(DDB_USER_EVENTS)
tbl_users  = dynamodb.Table(DDB_USERS)
tbl_search = dynamodb.Table(DDB_USER_SEARCH)
tbl_presence = dynamodb.Table(DDB_PRESENCE)
tbl_typing   = dynamodb.Table(DDB_TYPING)

# NEW:
tbl_edits = dynamodb.Table(DDB_MESSAGE_EDITS)
tbl_views = dynamodb.Table(DDB_MESSAGE_VIEWS)

app = FastAPI(title="Messaging API", version="0.5.0")
...
@app.post("/conversations/group", response_model=ConversationOut)
def start_group_conversation(inp: StartGroupConversationIn, user_id: str = Depends(get_current_user_id)):
    return start_conversation(StartConversationIn(
...
@app.post("/conversations/{conversation_id}/mute")
...
@app.post("/conversations/{conversation_id}/images/presign", response_model=PresignOut)
...
# React to message
# -------------------------
@app.post("/conversations/{conversation_id}/messages/{message_id}/reactions")
def react_to_message(conversation_id: str, message_id: str, inp: ReactIn, user_id: str = Depends(get_current_user_id)):
    ...
    fanout_event_to_conversation(
        ...
        payload={"message_id": message_id, "emoji": inp.emoji, "action": inp.action, "user_id": user_id, "updated_at": ts},
        respect_mute=False,
    )
    return {"ok": True}
...
def edit_message(conversation_id: str, message_id: str, inp: EditMessageIn, user_id: str = Depends(get_current_user_id)):
    ...
    # Read current (to capture old_text)
    ...
    if new_text == old_text:
        # still bump edited_at? Usually no.
        return MessageOut(
    ...
    # Write edit history entry (separate table)
    # MessageEdits schema: PK=message_key, SK=edited_at (number)
    tbl_edits.put_item(Item={
    ...
    try:
        tbl_msgs.update_item(
            ...
            ExpressionAttributeValues={":text": new_text, ":ts": ts, ":uid": user_id},
            ConditionExpression="sender_id = :uid AND kind = :kind_text",
            ExpressionAttributeValuesAdditional={},
        )
    except TypeError:
        # boto3 doesn't support ExpressionAttributeValuesAdditional; ignore.
        pass
    except Exception as e:
...
# -------------------------
@app.get("/conversations/{conversation_id}/messages/{message_id}/edits", response_model=List[EditHistoryOut])
def get_edit_history(
    ...
    # Ensure message exists (optional)
...
# Forward message
# -------------------------
@app.post("/conversations/{target_conversation_id}/messages/forward", response_model=MessageOut)
def forward_message(
    ...
    item: Dict[str, Any] = {
        ...
        "message_id": mid,
        "sender_id": user_id,  # who forwarded
        "created_at": ts,
        "kind": kind,
        "deleted_for": set(),
        "reactions": {},
...
# -------------------------
@app.post("/conversations/{conversation_id}/messages/{message_id}/view", response_model=ViewAckOut)
def mark_message_viewed(
    ...
    # normalize: don't allow far-future timestamps
    if ts > now_ts() + 300:
        ts = now_ts()
    ...
    tbl_views.update_item(
        Key=key,
        UpdateExpression="SET message_id = :mid, user_id = :uid, last_viewed_at = :ts, ttl = :ttl "
                         "ADD view_count :one",
    ...
    # Optional realtime signal (useful for “seen” UX)
    fanout_event_to_conversation(
        conversation_id=conversation_id,
        sender_id=user_id,
...
@app.get("/conversations/{conversation_id}/messages/{message_id}/views", response_model=List[MessageViewOut])
def get_message_views(
    ...
    resp = tbl_views.query(
        KeyConditionExpression=Key("conversation_id").eq(conversation_id) & Key("message_user").begins_with(f"{message_id}#"),
        Limit=limit,
        ScanIndexForward=True,
    )
    items = resp.get("Items", [])
...
@app.get("/conversations/{conversation_id}/typing", response_model=List[TypingUser])
...
@app.get("/presence", response_model=List[PresenceOut])
def presence_get(
    ...
    resp = dynamodb.batch_get_item(RequestItems={DDB_PRESENCE: {"Keys": keys}})
