...
# -----------------------------
# Pay balance via wallets (Apple Pay / Google Pay / Payment Request Button)
# -----------------------------
@app.post("/api/billing/pay-balance/wallet")
def create_wallet_pay_balance_intent(body: CreateWalletPayBalanceIn, x_user_id: Optional[str] = Header(default=None)):
    user_id = require_user(x_user_id)
    pk = user_pk(user_id)
...
async def stripe_webhook(req: Request):
    ...
    # ---- PaymentIntent status changes (success/fail/processing/canceled)
        ...
        if status == "processing":
            ...
            # If we haven't counted pending yet (e.g., PI started as requires_payment_method),
            # count it now.
            ...
            # Reverse what we previously counted, if anything.
            if prev_status in ("processing", "requires_action"):
                apply_balance_delta(pk, {"payments_pending_cents": -amount})
            elif prev_status == "succeeded":
                apply_balance_delta(pk, {"payments_settled_cents": -amount})
